      ...
      if nn<4*radius*radius:
        if nn !=0:
          if a[d*i+7]==0 and a[d*i+7]==0:
            ...
            a[d*i]=a[d*i]+(1+radius-sqrt(nn)/2)*n1/sqrt(nn)
            a[d*i+1]=a[d*i+1]+(1+radius-sqrt(nn)/2)*n2/sqrt(nn)
            a[d*j]=a[d*j]+(1+radius-sqrt(nn)/2)*n1/sqrt(nn)
            a[d*j+1]=a[d*j+1]+(1+radius-sqrt(nn)/2)*n2/sqrt(nn)
            a[d*i+7]=0
            a[d*j+7]=0
    if a[d*i+7]!=0:
      a[d*i+7]=a[d*i+7]-1
